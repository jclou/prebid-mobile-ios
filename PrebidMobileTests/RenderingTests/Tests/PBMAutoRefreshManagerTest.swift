...
class PBMAutoRefreshManagerTest: XCTestCase {
    func testNoTimerAutoStart() {
        ...
        withExtendedLifetime(Factory.PBMAutoRefreshManager(prefetchTime: 0,
                                                           refreshDelayBlock: { 5 },
    ...
    func testNoTimerAutoStartWithPrefetch() {
        let callbackExpectation = expectation(description: "callback called")
        callbackExpectation.isInverted = true
        withExtendedLifetime(Factory.PBMAutoRefreshManager(prefetchTime: 2,
                                                           refreshDelayBlock: { 4 },
                                                           mayRefreshNowBlock: { true },
                                                           refreshBlock: callbackExpectation.fulfill)) {
    ...
    func testInitialRequest() {
        let callbackExpectation = expectation(description: "callback called")
        let autoRefreshManager = Factory.PBMAutoRefreshManager(prefetchTime: 0,
                                                               refreshDelayBlock: { 5 },
                                                               mayRefreshNowBlock: { true },
                                                               refreshBlock: callbackExpectation.fulfill)
    ...
    func testInitialRequestWithPrefetch() {
        let callbackExpectation = expectation(description: "callback called")
        let autoRefreshManager = Factory.PBMAutoRefreshManager(prefetchTime: 2,
                                                               refreshDelayBlock: { 6 },
                                                               mayRefreshNowBlock: { true },
                                                               refreshBlock: callbackExpectation.fulfill)
        let beforeSetup = Date()
    ...
    func testNoRepeatedRequest() {
        ...
        let autoRefreshManager = Factory.PBMAutoRefreshManager(prefetchTime: 0, refreshDelayBlock: {3}, mayRefreshNowBlock: {true}) {
    ...
    func testNoRepeatedRequestWithPrefetch() {
        let callbackExpectation = expectation(description: "callback called")
        let nextExpectation = NSMutableArray(object: callbackExpectation)
        let autoRefreshManager = Factory.PBMAutoRefreshManager(prefetchTime: 2, refreshDelayBlock: {6}, mayRefreshNowBlock: {true}) {
            (nextExpectation[0] as? XCTestExpectation)?.fulfill()
        }
        autoRefreshManager.setupRefreshTimer()
        waitForExpectations(timeout: 5)
                                         ...
                                         extraTimeToFinish: TimeInterval = 1)
    {
        ...
        let autoRefreshManager = Factory.PBMAutoRefreshManager(prefetchTime: prefetchTime,
                                                               refreshDelayBlock: {
