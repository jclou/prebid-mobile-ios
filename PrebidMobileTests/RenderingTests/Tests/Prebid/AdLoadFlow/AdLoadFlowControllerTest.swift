...
class AdLoadFlowControllerTest: XCTestCase {
    ...
    func testNoImmediateCalls() {
        func TestCast(_ x: AdLoadFlowController) -> (AdLoadFlowController & PBMAdLoadFlowController_Objc) {
            x as! (AdLoadFlowController & PBMAdLoadFlowController_Objc)
        }
        
        let adUnitConfig = AdUnitConfig(configId: "configId")
        let compositeMock = CompositeMock(expectedCalls: [])
        let flowController = Factory.AdLoadFlowController(bidRequesterFactory: compositeMock.mockRequesterFactory,
        ...
        XCTAssertEqual(TestCast(flowController).flowState, .idle)
    ...
    func testPrimaryAd_happyPath(preFailed: Bool) {
        ...
        flowController = Factory.AdLoadFlowController(bidRequesterFactory: compositeMock.mockRequesterFactory,
                                                      adLoader: compositeMock.mockAdLoader,
                                                      adUnitConfig: adUnitConfig,
                                                      delegate: compositeMock.mockFlowControllerDelegate,
                                                      configValidationBlock: compositeMock.mockConfigValidator)
        ...
        if (preFailed) {
            Impl(flowController).flowState = .loadingFailed
        }
        ...
        XCTAssertFalse(flowController.hasFailedLoading)
        XCTAssertEqual(Impl(flowController).flowState, .idle)
        compositeMock.checkIsFinished()
    }
    ...
    func testPrimaryAd_noBids_noPrimaryAd() {
        ...
        XCTAssertEqual(Impl(flowController).flowState, .loadingFailed)
        compositeMock.checkIsFinished()
    }
    ...
    func testPrebidAd_happyPath_freezeOnShouldContinue() {
        ...
        XCTAssertEqual(Impl(flowController).flowState, .demandReceived)
        ...
        XCTAssertEqual(Impl(flowController).flowState, .readyToDeploy)
